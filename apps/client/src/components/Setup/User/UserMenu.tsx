...
export default function UserMenu(props: SetupProps) {
  ...
  function reloadSetter() {
    ...
    props.gRPCClients.teamClient.getAll(new GetAllRequestTeam(), {}).then(
      (teamsResponse) => {
        for (let i = 0; i < teamsResponse.getTeamsList().length; i++) {
          lookup[
            teamsResponse.getTeamsList()[i].getId()?.getValue() as string
          ] = `${teamsResponse.getTeamsList()[i].getName()} (ID:${
            teamsResponse.getTeamsList()[i].getId()?.getValue() as string
          })`;
        }
        setState((prevState) => {
    ...
    props.gRPCClients.userClient.getAll(new GetAllRequestUser(), {}).then(
      (usersResponse) => {
        setState((prevState) => {
          return {
            ...prevState,
            data: usersResponse.getUsersList().map((user): userColumns => {
              return userToUserColumn(user);
            }),
            loaderUser: false,
          };
        });
      ...
      (err: any) => {
        props.genericEnqueue(
          `Encountered an error while retrieving Users: ${err.message}. Error code: ${err.code}`,
          Severity.Error
        );
      }
    );
              ...
              onRowAdd: (newData) =>
                new Promise<void>((resolve, reject) => {
                  setTimeout(() => {
                    ...
                    props.gRPCClients.userClient.store(storeRequest, {}).then(
                      (result) => {
                      ...
                      (err: any) => {
                        props.genericEnqueue(
                          `Unable to store user: ${err.message}. Error code: ${err.code}`,
                          Severity.Error
                        );
                        reject();
                      }
                    ...
                    if (oldData) {
                      ...
                      props.gRPCClients.userClient
                        ...
                        .then(
                          ...
                          (err: any) => {
                            props.genericEnqueue(
                              `Unable to update user: ${err.message}. Error code: ${err.code}`,
                              Severity.Error
                            );
                            reject();
                          }
              ...
              onRowDelete: (oldData) =>
                new Promise<void>((resolve, reject) => {
                  setTimeout(() => {
                    ...
                    props.gRPCClients.userClient.delete(deleteRequest, {}).then(
                      (result) => {
                        setState((prevState) => {
                          const data = [...prevState.data];
                      ...
                      (err: any) => {
                        props.genericEnqueue(
                          `Unable to delete user: ${err.message}. Error code: ${err.code}`,
                          Severity.Error
                        );
                        reject();
                      }
