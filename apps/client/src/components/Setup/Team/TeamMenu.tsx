...
export default function TeamMenu(props: SetupProps) {
  ...
  return (
    <Box height="100%" width="100%">
      ...
      <div>
        {activeStep === 0 && <TeamMenuTable {...props} />}
        {activeStep === 1 && <TeamCreate {...props} />}
      </div>
    </Box>
  );
}
...
export function teamColumnsToTeam(teamC: teamColumns): Team {
  ...
  if (teamC.index != null && !isNaN(teamC.index))
    t.setIndex(new UInt64Value().setValue(teamC.index));
  return t;
}
...
function TeamMenuTable(props: SetupProps) {
  ...
  function reloadSetter() {
    props.gRPCClients.teamClient.getAll(new GetAllRequest(), {}).then(
      (teamsResponse) => {
        setState((prevState) => {
          return {
            ...prevState,
            data: teamsResponse.getTeamsList().map((team): teamColumns => {
              ...
              onRowAdd: (newData) =>
                new Promise<void>((resolve, reject) => {
                  setTimeout(() => {
                    ...
                    props.gRPCClients.teamClient.store(storeRequest, {}).then(
                      (result) => {
                      ...
                      (err: any) => {
                        props.genericEnqueue(
                          `Unable to store team: ${err.message}. Error code: ${err.code}`,
                          Severity.Error
                        );
                        reject();
                      }
                    ...
                    if (oldData) {
                      ...
                      props.gRPCClients.teamClient
                        ...
                        .then(
                          ...
                          (err: any) => {
                            props.genericEnqueue(
                              `Unable to update team: ${err.message}. Error code: ${err.code}`,
                              Severity.Error
                            );
                            reject();
                          }
              ...
              onRowDelete: (oldData) =>
                new Promise<void>((resolve, reject) => {
                  setTimeout(() => {
                    ...
                    props.gRPCClients.teamClient.delete(deleteRequest, {}).then(
                      (result) => {
                        setState((prevState) => {
                          const data = [...prevState.data];
                      ...
                      (err: any) => {
                        props.genericEnqueue(
                          `Unable to delete team: ${err.message}. Error code: ${err.code}`,
                          Severity.Error
                        );
                        reject();
                      }
