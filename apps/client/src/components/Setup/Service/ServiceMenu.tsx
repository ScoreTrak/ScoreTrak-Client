...
import { GetAllRequest as GetAllRequestHost } from "@scoretrak/scoretrakapis/scoretrak/host/v1/host_pb";
import {
  DeleteRequest,
  GetAllRequest,
  Service,
  ...
  TestServiceRequest,
  UpdateRequest,
} from "@scoretrak/scoretrakapis/scoretrak/service/v1/service_pb";
import {
  BoolValue,
  UInt64Value,
} from "google-protobuf/google/protobuf/wrappers_pb";
import { GetAllRequest as GetAllRequestServiceGroup } from "@scoretrak/scoretrakapis/scoretrak/service_group/v1/service_group_pb";
...
export default function ServiceMenu(props: SetupProps) {
  ...
  return (
    <Box height="100%" width="100%">
      ...
      <div>
        {activeStep === 0 && <ServiceMenuTable {...props} />}
        {activeStep === 1 && <ServiceCreate {...props} />}
      </div>
    </Box>
  );
}
...
export function serviceColumnsToService(serviceC: serviceColumns): Service {
  const u = new Service();
  if (serviceC.id && serviceC.id !== "")
    u.setId(new UUID().setValue(serviceC.id));
  u.setDisplayName(serviceC.displayName);
  if (serviceC.hostId && serviceC.hostId !== "")
    u.setHostId(new UUID().setValue(serviceC.hostId));
  if (serviceC.serviceGroupId && serviceC.serviceGroupId !== "")
    u.setServiceGroupId(new UUID().setValue(serviceC.serviceGroupId));
  if (serviceC.pause != null)
    u.setPause(new BoolValue().setValue(serviceC.pause));
  if (serviceC.hide != null) u.setHide(new BoolValue().setValue(serviceC.hide));
  u.setName(serviceC.name);
  if (serviceC.weight != null && !isNaN(serviceC.weight))
    u.setWeight(new UInt64Value().setValue(serviceC.weight));
  u.setRoundUnits(serviceC.roundUnits);
  if (serviceC.roundDelay != null && !isNaN(serviceC.roundDelay))
    u.setRoundDelay(new UInt64Value().setValue(serviceC.roundDelay));
  if (serviceC.pointsBoost != null && !isNaN(serviceC.pointsBoost))
    u.setPointsBoost(new UInt64Value().setValue(serviceC.pointsBoost));
  return u;
}
...
function ServiceMenuTable(props: SetupProps) {
  ...
  const [state, setState] = React.useState<{
    columns: any[];
    loaderServiceGroup: boolean;
    loaderService: boolean;
    loaderHost: boolean;
    data: serviceColumns[];
  }>({
    columns,
  ...
  function reloadSetter() {
    props.gRPCClients.hostClient.getAll(new GetAllRequestHost(), {}).then(
      (hostsResponse) => {
        const lookup: Record<string, string> = {};
        for (let i = 0; i < hostsResponse.getHostsList().length; i++) {
          lookup[
            hostsResponse.getHostsList()[i].getId()?.getValue() as string
          ] = `${hostsResponse.getHostsList()[i].getAddress()} (ID:${
            hostsResponse.getHostsList()[i].getId()?.getValue() as string
          })`;
        }
        setState((prevState) => {
      ...
      (err: any) => {
        props.genericEnqueue(
          `Encountered an error while retrieving parent Teams: ${err.message}. Error code: ${err.code}`,
          Severity.Error
        );
      }
    );

    props.gRPCClients.serviceGroupClient
      ...
      .then(
        (serviceGroupRequest) => {
          ...
          for (
            let i = 0;
            i < serviceGroupRequest.getServiceGroupsList().length;
            i++
          ) {
            lookup[
              serviceGroupRequest
    ...
    props.gRPCClients.serviceClient.getAll(new GetAllRequest(), {}).then(
      (servicesResponse) => {
        setState((prevState) => {
          return {
            ...prevState,
      ...
      (err: any) => {
        props.genericEnqueue(
          `Encountered an error while retrieving Services: ${err.message}. Error code: ${err.code}`,
          Severity.Error
        );
      }
    );
            ...
            actions={[
              {
                ...
                onClick: (event, rowData) => {
                  return props.gRPCClients.serviceClient
                    .testService(
                      new TestServiceRequest().setId(
                        new UUID().setValue(
                          (rowData as serviceColumns).id as string
                        )
                      ),
                      {}
                    )
              ...
              onRowAdd: (newData) =>
                new Promise<void>((resolve, reject) => {
                  setTimeout(() => {
                    ...
                    props.gRPCClients.serviceClient
                      .store(storeRequest, {})
                      .then(
                        (result) => {
                        ...
                        (err: any) => {
                          props.genericEnqueue(
                            `Unable to store service: ${err.message}. Error code: ${err.code}`,
                            Severity.Error
                          );
                          reject();
                        }
                    ...
                    if (oldData) {
                      ...
                      props.gRPCClients.serviceClient
                        ...
                        .then(
                          ...
                          (err: any) => {
                            props.genericEnqueue(
                              `Unable to update service: ${err.message}. Error code: ${err.code}`,
                              Severity.Error
                            );
                            reject();
                          }
                  ...
                  setTimeout(() => {
                    ...
                    props.gRPCClients.serviceClient
                      ...
                      .then(
                        ...
                        (err: any) => {
                          props.genericEnqueue(
                            `Unable to delete service: ${err.message}. Error code: ${err.code}`,
                            Severity.Error
                          );
                          reject();
                        }
