...
function hostGroupColumnsToHostGroup(hostGroupC: hostGroupColumns): HostGroup {
  ...
  if (hostGroupC.pause != null)
    t.setPause(new BoolValue().setValue(hostGroupC.pause));
  if (hostGroupC.hide != null)
    t.setHide(new BoolValue().setValue(hostGroupC.hide));
  if (hostGroupC.id && hostGroupC.id !== "")
    t.setId(new UUID().setValue(hostGroupC.id));
...
export default function HostGroupsMenu(props: SetupProps) {
  ...
  function reloadSetter() {
    props.gRPCClients.hostGroupClient.getAll(new GetAllRequest(), {}).then(
      (hostGroupsResponse) => {
        setState((prevState) => {
          return {
            ...prevState,
            data: hostGroupsResponse
              .getHostGroupsList()
              .map((hostGroup): hostGroupColumns => {
                return hostGroupToHostGroupColumns(hostGroup);
              }),
            loader: false,
          };
        });
                  ...
                  setTimeout(() => {
                    ...
                    props.gRPCClients.hostGroupClient
                      ...
                      .then(
                        ...
                        (err: any) => {
                          props.genericEnqueue(
                            `Unable to store hostGroup: ${err.message}. Error code: ${err.code}`,
                            Severity.Error
                          );
                          reject();
                        }
                    ...
                    if (oldData) {
                      ...
                      props.gRPCClients.hostGroupClient
                        ...
                        .then(
                          ...
                          (err: any) => {
                            props.genericEnqueue(
                              `Unable to update hostGroup: ${err.message}. Error code: ${err.code}`,
                              Severity.Error
                            );
                            reject();
                          }
                  ...
                  setTimeout(() => {
                    ...
                    props.gRPCClients.hostGroupClient
                      ...
                      .then(
                        ...
                        (err: any) => {
                          props.genericEnqueue(
                            `Unable to delete hostGroup: ${err.message}. Error code: ${err.code}`,
                            Severity.Error
                          );
                          reject();
                        }
