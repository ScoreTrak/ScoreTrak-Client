...
export function hostColumnsToHost(hostC: hostColumns): Host {
  ...
  if (hostC.hostGroupId && hostC.hostGroupId !== "")
    u.setHostGroupId(new UUID().setValue(hostC.hostGroupId));
  if (hostC.teamId && hostC.teamId !== "")
    u.setTeamId(new UUID().setValue(hostC.teamId));
  ...
  if (hostC.editHost != null)
    u.setEditHost(new BoolValue().setValue(hostC.editHost));
  if (hostC.addressListRange != null)
    u.setAddressListRange(new StringValue().setValue(hostC.addressListRange));
...
function HostMenuTable(props: SetupProps) {
  ...
  function reloadSetter() {
    props.gRPCClients.hostGroupClient
      ...
      .then(
        (hostsGroupResponse) => {
          ...
          for (
            let i = 0;
            i < hostsGroupResponse.getHostGroupsList().length;
            i++
          ) {
            lookup[
              hostsGroupResponse
                ...
                [i].getId()
                ?.getValue() as string
            ] = `${hostsGroupResponse.getHostGroupsList()[i].getName()} (ID:${
              hostsGroupResponse
                .getHostGroupsList()
                [i].getId()
                ?.getValue() as string
        ...
        (err: any) => {
          props.genericEnqueue(
            `Encountered an error while retrieving parent Host Groups: ${err.message}. Error code: ${err.code}`,
            Severity.Error
          );
        }
      );
    ...
    props.gRPCClients.teamClient.getAll(new GetAllRequestTeam(), {}).then(
      (teamResponse) => {
        ...
        for (let i = 0; i < teamResponse.getTeamsList().length; i++) {
          lookup[
            teamResponse.getTeamsList()[i].getId()?.getValue() as string
          ] = `${teamResponse.getTeamsList()[i].getName()} (ID:${
            teamResponse.getTeamsList()[i].getId()?.getValue() as string
          })`;
        }
        setState((prevState) => {
    ...
    props.gRPCClients.hostClient.getAll(new GetAllRequest(), {}).then(
      (hostsResponse) => {
        setState((prevState) => {
          return {
            ...prevState,
            data: hostsResponse.getHostsList().map((host): hostColumns => {
      ...
      (err: any) => {
        props.genericEnqueue(
          `Encountered an error while retrieving Hosts: ${err.message}. Error code: ${err.code}`,
          Severity.Error
        );
      }
    );
                ...
                new Promise<void>((resolve, reject) => {
                  setTimeout(() => {
                    ...
                    props.gRPCClients.hostClient.store(storeRequest, {}).then(
                      ...
                      (err: any) => {
                        props.genericEnqueue(
                          `Unable to store Host: ${err.message}. Error code: ${err.code}`,
                          Severity.Error
                        );
                        reject();
                      }
                    ...
                    if (oldData) {
                      ...
                      props.gRPCClients.hostClient
                        ...
                        .then(
                          ...
                          (err: any) => {
                            props.genericEnqueue(
                              `Unable to update hosts: ${err.message}. Error code: ${err.code}`,
                              Severity.Error
                            );
                            reject();
                          }
              ...
              onRowDelete: (oldData) =>
                new Promise<void>((resolve, reject) => {
                  setTimeout(() => {
                    ...
                    props.gRPCClients.hostClient.delete(deleteRequest, {}).then(
                      (result) => {
                        setState((prevState) => {
                          const data = [...prevState.data];
                      ...
                      (err: any) => {
                        props.genericEnqueue(
                          `Unable to delete host: ${err.message}. Error code: ${err.code}`,
                          Severity.Error
                        );
                        reject();
                      }
