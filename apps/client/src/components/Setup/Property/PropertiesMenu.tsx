...
import { Status as ProtoStatus } from "@scoretrak/scoretrakapis/scoretrak/property/v1/property_pb";
import { GetAllRequest as GetAllRequestService } from "@scoretrak/scoretrakapis/scoretrak/service/v1/service_pb";
...
export default function PropertiesMenu(props: SetupProps) {
  ...
  return (
    <Box height="100%" width="100%">
      ...
      <div>
        {activeStep === 0 && <PropertyMenuTable {...props} />}
        {activeStep === 1 && <PropertiesCreate {...props} />}
      </div>
    </Box>
  );
}
...
export function propertyColumnsToProperty(
  propertyC: propertyColumns
): Property {
  const u = new Property();
  if (propertyC.serviceId && propertyC.serviceId !== "")
    u.setServiceId(new UUID().setValue(propertyC.serviceId));
  u.setKey(propertyC.key);
  if (propertyC.value && propertyC.value !== "")
    u.setValue(new UUID().setValue(propertyC.value));
...
function PropertyMenuTable(props: SetupProps) {
  ...
  function reloadSetter() {
    props.gRPCClients.serviceClient.getAll(new GetAllRequestService(), {}).then(
      (serviceResponse) => {
        const lookup: Record<string, string> = {};
        for (let i = 0; i < serviceResponse.getServicesList().length; i++) {
          lookup[
            serviceResponse.getServicesList()[i].getId()?.getValue() as string
          ] = `${serviceResponse.getServicesList()[i].getDisplayName()} (ID:${
            serviceResponse.getServicesList()[i].getId()?.getValue() as string
          })`;
        }
        setState((prevState) => {
    ...
    props.gRPCClients.propertyClient.getAll(new GetAllRequest(), {}).then(
      (propertiesResponse) => {
        setState((prevState) => {
          return {
            ...prevState,
      ...
      (err: any) => {
        props.genericEnqueue(
          `Encountered an error while retrieving Properties: ${err.message}. Error code: ${err.code}`,
          Severity.Error
        );
      }
    );
              ...
              onRowAdd: (newData) =>
                new Promise<void>((resolve, reject) => {
                  setTimeout(() => {
                    ...
                    props.gRPCClients.propertyClient
                      .store(storeRequest, {})
                      .then(
                        (result) => {
                          setState((prevState) => {
                        ...
                        (err: any) => {
                          props.genericEnqueue(
                            `Unable to store property: ${err.message}. Error code: ${err.code}`,
                            Severity.Error
                          );
                          reject();
                        }
                    ...
                    if (oldData) {
                      ...
                      props.gRPCClients.propertyClient
                        ...
                        .then(
                          ...
                          (err: any) => {
                            props.genericEnqueue(
                              `Unable to update property: ${err.message}. Error code: ${err.code}`,
                              Severity.Error
                            );
                            reject();
                          }
                  ...
                  setTimeout(() => {
                    ...
                    props.gRPCClients.propertyClient
                      ...
                      .then(
                        ...
                        (err: any) => {
                          props.genericEnqueue(
                            `Unable to delete property: ${err.message}. Error code: ${err.code}`,
                            Severity.Error
                          );
                          reject();
                        }
