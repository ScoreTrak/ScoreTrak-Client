...
function serviceGroupToServiceGroupColumn(
...
function serviceGroupColumnsToServiceGroup(
  serviceGroupC: serviceGroupColumns
): ServiceGroup {
  const t = new ServiceGroup();
  if (serviceGroupC.enabled != null)
    t.setEnabled(new BoolValue().setValue(serviceGroupC.enabled));
  if (serviceGroupC.id && serviceGroupC.id !== "")
    t.setId(new UUID().setValue(serviceGroupC.id));
...
export function ServiceGroupsMenu(props: SetupProps) {
  ...
  function reloadSetter() {
    props.gRPCClients.serviceGroupClient.getAll(new GetAllRequest(), {}).then(
      (serviceGroupResponse) => {
        setState((prevState) => {
          return {
            ...prevState,
            data: serviceGroupResponse
              .getServiceGroupsList()
              .map((service): serviceGroupColumns => {
                return serviceGroupToServiceGroupColumn(service);
              }),
            loader: false,
          };
        });
            ...
            actions={[
              {
                ...
                onClick: (event, rowData) => {
                  return props.gRPCClients.serviceGroupClient
                    .redeploy(
                      new RedeployRequest().setId(
                        new UUID().setValue(
                          (rowData as serviceGroupColumns).id as string
                        )
                      ),
                      {}
                    )
                  ...
                  setTimeout(() => {
                    ...
                    props.gRPCClients.serviceGroupClient
                      ...
                      .then(
                        ...
                        (err: any) => {
                          props.genericEnqueue(
                            `Unable to store service group: ${err.message}. Error code: ${err.code}`,
                            Severity.Error
                          );
                          reject();
                        }
                    ...
                    if (oldData) {
                      ...
                      props.gRPCClients.serviceGroupClient
                        ...
                        .then(
                          ...
                          (err: any) => {
                            props.genericEnqueue(
                              `Unable to update service group: ${err.message}. Error code: ${err.code}`,
                              Severity.Error
                            );
                            reject();
                          }
                  ...
                  setTimeout(() => {
                    ...
                    props.gRPCClients.serviceGroupClient
                      ...
                      .then(
                        ...
                        (err: any) => {
                          props.genericEnqueue(
                            `Unable to delete Service Group: ${err.message}. Error code: ${err.code}`,
                            Severity.Error
                          );
                          reject();
                        }
