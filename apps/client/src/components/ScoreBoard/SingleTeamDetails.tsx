                  ...
                  nextState[service_id] = [
                    {
                      ...
                      passed:
                        prevDT.Teams[teamID].Hosts[host].Services[service_id]
                          .Check?.Passed,
                      err: prevDT.Teams[teamID].Hosts[host].Services[service_id]
                        .Check?.Err,
                      log: prevDT.Teams[teamID].Hosts[host].Services[service_id]
                        .Check?.Log,
                      round_id: prevDT.Round,
                    },
                  ];
    ...
    <Box height="100%" width="100%">
      {Object.keys(props.report.Teams[teamID].Hosts).map((host) => {
        ...
        return Object.keys(currentHost.Services).map((service_id) => {
          ...
          simpleService.Pause =
            props.report.Teams[teamID].Pause ||
...
function SingleTeamDetailsAccordionDetailsBox(
  ...
  async function reloadProperties(
    ...
    return props.gRPCClients.propertyClient.getAllByServiceID(
      propertiesRequest,
      {}
    );
  }
  ...
  const handleSetHostAddress = (
    ...
    props.gRPCClients.hostClient.update(hostsRequest, {}).then(
      (r) => {
