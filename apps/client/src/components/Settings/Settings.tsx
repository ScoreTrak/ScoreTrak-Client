...
} from "@scoretrak/scoretrakapis/scoretrak/config/v1/config_pb";
...
import {
  ...
  ResetScoresRequest,
} from "@scoretrak/scoretrakapis/scoretrak/competition/v1/competition_pb";
import { TransitionProps } from "@mui/material/transitions";
import { Report } from "@scoretrak/scoretrakapis/scoretrak/report/v1/report_pb";
import { Timestamp } from "google-protobuf/google/protobuf/timestamp_pb";
import { Check } from "@scoretrak/scoretrakapis/scoretrak/check/v1/check_pb";
...
import { Property } from "@scoretrak/scoretrakapis/scoretrak/property/v1/property_pb";
...
export default function Settings(
  props: SetupProps & { currentPolicy: Policy.AsObject }
) {
  ...
  function loadAll() {
    props.gRPCClients.staticConfigClient
      .get(new GetStaticConfigRequest(), {})
      .then(
        (response) => {
          setData((prevState) => {
            return {
    ...
    props.gRPCClients.dynamicConfigClient.get(new GetRequest(), {}).then(
      (response) => {
        setData((prevState) => {
          return {
            ...prevState,
  ...
  const handleSetEnabled = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    props.gRPCClients.dynamicConfigClient
      .update(
  ...
  const handleSetPolicy = (policy: Policy) => {
    props.gRPCClients.policyClient
  ...
  const handleUpload = () => {
    ...
    if (file.files && file.files[0]) {
      ...
      reader.onload = function (evt) {
        ...
        props.gRPCClients.competitionClient
          .loadCompetition(
  ...
  const handleResetCompetition = () => {
    props.gRPCClients.competitionClient
      ...
      .then(
        (resp) => {
          props.genericEnqueue(
            "Successfully reset all of the scores!",
            Severity.Success
          );
        },
        (err: any) => {
  ...
  const handleDeleteCompetition = () => {
    props.gRPCClients.competitionClient
      ...
      .then(
        (resp) => {
          props.genericEnqueue(
            "Successfully deleted all competition data!",
            Severity.Success
          );
        },
        (err: any) => {
              ...
              <Box p={1} m={1} bgcolor="background.paper">
                <FormControlLabel
                  control={
                    <Switch
                      checked={
                        props.currentPolicy.allowUnauthenticatedUsers?.value
                      }
                ...
                <FormControlLabel
                  control={
                    <Switch
                      checked={
                        props.currentPolicy.allowChangingUsernamesAndPasswords
                          ?.value
                      }
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                        handleSetPolicy(
                ...
                <FormControlLabel
                  control={
                    <Switch
                      checked={props.currentPolicy.showPoints?.value}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                        handleSetPolicy(
                ...
                <FormControlLabel
                  control={
                    <Switch
                      checked={props.currentPolicy.showAddresses?.value}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                        handleSetPolicy(
                ...
                <FormControlLabel
                  control={
                    <Switch
                      checked={
                        props.currentPolicy
                          .allowRedTeamLaunchingServiceTestsManually?.value
                      }
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                        handleSetPolicy(
            ...
            <AccordionDetails>
              ...
              >
                <ReactJson
                  ...
                  theme={props.isDarkTheme ? "monokai" : "bright:inverted"}
                />
              </Box>
            </AccordionDetails>
          </Accordion>
