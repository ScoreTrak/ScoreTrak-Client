...
import {
    Competition, DeleteCompetitionRequest, FetchCoreCompetitionRequest, FetchEntireCompetitionRequest,
...
export default function Settings() {
    ...
    const [fileSelected, setFileSelected] = useState({selected: false, name: ""});
    ...
    function loadAll(){
        gRPCClients.staticConfigClient.get(new GetStaticConfigRequest(), {}).then(response => {
        ...
        gRPCClients.dynamicConfigClient.get(new GetRequest(), {}).then(response => {
    ...
    const handleSetEnabled = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        gRPCClients.dynamicConfigClient.update(new UpdateRequestDynamicConfig().setDynamicConfig(new DynamicConfig().setEnabled(boolVal)), {}).then(resp => {
            setData(prevState => {
    ...
    const handleSetRoundDuration = (e: React.SyntheticEvent) => {
        ...
        gRPCClients.dynamicConfigClient.update(new UpdateRequestDynamicConfig().setDynamicConfig(new DynamicConfig().setRoundDuration(val)), {}).then(resp => {
            setData(prevState => {
                const newDN = prevState.dynamicConfig
    ...
    const handleSetPolicy = (policy: Policy) => {
        gRPCClients.policyClient.update(new UpdateRequestPolicy().setPolicy(policy), {}).then(resp => {}, (err: any) => {
    ...
    const handleUpload = () => {
        ...
        if (file.files && file.files[0]){
            ...
            reader.onload = function (evt){
                ...
                gRPCClients.competitionClient.loadCompetition(new LoadCompetitionRequest().setCompetition(comp), {}).then(resp => {
    ...
    const handleResetCompetition = () => {
        gRPCClients.competitionClient.resetScores(new ResetScoresRequest(), {}).then(resp => {
    ...
    const handleDeleteCompetition = () => {
        gRPCClients.competitionClient.deleteCompetition(new DeleteCompetitionRequest(), {}).then(resp => {
                        ...
                        <AccordionDetails>
                            <Box width="100%" p={1} m={1} bgcolor="background.paper" textAlign="center">
                                <Button
                                    ...
                                    startIcon={<SaveIcon />}
                                    onClick={() => {
                                        gRPCClients.competitionClient.fetchCoreCompetition(new FetchCoreCompetitionRequest(), {}).then(resp => {
                                ...
                                <Button
                                    ...
                                    className={classes.button}
                                    startIcon={<SaveIcon />}
                                    onClick={() => {gRPCClients.competitionClient.fetchEntireCompetition(new FetchEntireCompetitionRequest(), {}).then(resp => {
