...
export function IHostGroupToHostGroup(hostGroup: IHostGroup): HostGroup {
  ...
  if (hostGroup.pause != null)
    t.setPause(new BoolValue().setValue(hostGroup.pause));
  if (hostGroup.hide != null)
    t.setHide(new BoolValue().setValue(hostGroup.hide));
  if (hostGroup.id && hostGroup.id !== "")
    t.setId(new UUID().setValue(hostGroup.id));
