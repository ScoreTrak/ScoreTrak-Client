...
export function IServiceGroupToServiceGroup(
  serviceGroup: IServiceGroup
): ServiceGroup {
  const t = new ServiceGroup();
  if (serviceGroup.enabled != null)
    t.setEnabled(new BoolValue().setValue(serviceGroup.enabled));
  if (serviceGroup.id && serviceGroup.id !== "")
    t.setId(new UUID().setValue(serviceGroup.id));
