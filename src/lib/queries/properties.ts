...
export function usePropertiesQuery() {
    const fetchProperties = async () => {
        const propertiesResponse = await gRPCClients.propertyClient.getAll(new GetAllRequest(), {})
...
export function usePropertiesByServiceIdQuery(serviceId: string) {
    const fetchPropertiesByServiceId = async (id: string) => {
        ...
        const propertiesResponse = await gRPCClients.propertyClient.getAllByServiceID(request, {})
        return propertiesResponse.getPropertiesList()
    }

    return useQuery<Property[], grpcWeb.RpcError>(["properties", serviceId], () => fetchPropertiesByServiceId(serviceId))
}
