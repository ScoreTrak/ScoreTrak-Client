...
export function useHostGroupsQuery() {
    const fetchHostGroups = async () => {
        const hostGroupsResponse = await gRPCClients.hostGroupClient.getAll(new GetAllRequest(), {})
        return hostGroupsResponse.getHostGroupsList()
    }

    return useQuery<HostGroup[], grpcWeb.RpcError>(["host-groups"], fetchHostGroups)
}
...
export function useHostGroupQuery(hostGroupId: string) {
    const fetchHostGroupById = async (id: string) => {
        ...
        request.setId(uuid)
        const hostGroupResponse = await gRPCClients.hostGroupClient.getByID(request, {})
        return hostGroupResponse.getHostGroup()
    }

    return useQuery<HostGroup | undefined, grpcWeb.RpcError>(["host-groups", hostGroupId], () => fetchHostGroupById(hostGroupId))
}
...
export function useDeleteHostGroupMutation() {
    ...
    const deleteHostGroup = async (deleteHostGroupRequest: DeleteRequest) => {
        const deleteResponse = await gRPCClients.hostGroupClient.delete(deleteHostGroupRequest, {})
