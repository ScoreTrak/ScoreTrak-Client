...
export function useUsersQuery() {
    const fetchUsers = async () => {
        const userResponse = await gRPCClients.userClient.getAll(new GetAllRequest(), {})
...
export function useUserQuery(userId: string) {
    ...
    return useQuery<User | undefined, grpcWeb.RpcError>(["users", userId], () => fetchUserById(userId))
}
...
export function useDeleteUserMutation() {
    ...
    const deleteUser = async (deleteUserRequest: DeleteRequest) => {
        const deleteResponse = await gRPCClients.userClient.delete(deleteUserRequest, {})
