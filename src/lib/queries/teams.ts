...
export function useTeamsQuery() {
    const fetchTeams = async () => {
        const teamsResponse = await gRPCClients.teamClient.getAll(new GetAllRequest(), {})
...
export function useTeamQuery(teamId: string) {
    ...
    return useQuery<Team | undefined, grpcWeb.RpcError>(["teams", teamId], () => fetchTeamById(teamId))
}
...
export function useDeleteTeamMutation() {
    ...
    const deleteTeam = async (deleteTeamRequest: DeleteRequest) => {
        const deleteResponse = await gRPCClients.teamClient.delete(deleteTeamRequest, {})
