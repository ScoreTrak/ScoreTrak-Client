...
export function useServicesQuery() {
    const fetchServices = async () => {
        const serviceResponse = await gRPCClients.serviceClient.getAll(new GetAllRequest(), {})
...
export function useServiceQuery(serviceId: string) {
    const fetchServiceById = async (id: string) => {
        ...
        request.setId(uuid)
        const serviceResponse = await gRPCClients.serviceClient.getByID(request, {})
        return serviceResponse.getService()
    }

    return useQuery<Service | undefined, grpcWeb.RpcError>(["services", serviceId], () => fetchServiceById(serviceId))
}
...
export function useDeleteServiceMutation() {
    ...
    const deleteService = async (deleteServiceRequest: DeleteRequest) => {
        const deleteResponse = await gRPCClients.serviceClient.delete(deleteServiceRequest, {})
