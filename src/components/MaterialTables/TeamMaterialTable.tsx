...
import {useAddTeamMutation, useDeleteTeamMutation, useTeamsQuery, useUpdateTeamMutation} from "../../lib/queries/teams";
                                ...
                                new Promise<void>((resolve, reject) => {
                                    setTimeout(() => {
                                        ...
                                        addTeam.mutate(storeRequest, {
                                            onError: (error) => {
                                                enqueueSnackbar(`Unable to store team: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, {
                                    ...
                                    setTimeout(() => {
                                        if (oldData) {
                                            ...
                                            updateTeam.mutate(updateRequest, {
                                                onError: (error) => {
                                                    enqueueSnackbar(`Unable to update team: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, {
                                                        variant: Severity.Error,
                                                        action: SnackbarDismissButton
                                                    })
                                                    reject()
                                ...
                                new Promise<void>((resolve, reject) => {
                                    setTimeout(() => {
                                        ...
                                        deleteTeam.mutate(deleteRequest, {
                                            onError: (error) => {
                                                enqueueSnackbar(`Unable to delete team: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, {
                                                    variant: Severity.Error,
                                                    action: SnackbarDismissButton
                                                })
                                                reject()
