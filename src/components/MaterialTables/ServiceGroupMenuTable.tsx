...
import {
    useAddServiceGroupMutation, useDeleteServiceGroupMutation,
...
import {IServiceGroupToServiceGroup, serviceGroupToIServiceGroup} from "../../lib/material-table/service-groups";
                        ...
                        actions={[
                            {
                                ...
                                onClick: (event, rowData) => {
                                    return gRPCClients.serviceGroupClient.redeploy(new RedeployRequest().setId(new UUID().setValue((rowData as IServiceGroup).id as string)), {}).then(() => {
                                ...
                                new Promise<void>((resolve, reject) => {
                                    setTimeout(() => {
                                        ...
                                        addServiceGroup.mutate(storeRequest, {
                                            onError: (error) => {
                                                enqueueSnackbar(`Unable to store service group: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, { variant: Severity.Error, action: SnackbarDismissButton })
                                                reject()
                                            }
                                        })
                                        resolve()
                                    ...
                                    setTimeout(() => {
                                        if (oldData){
                                            ...
                                            updateServiceGroup.mutate(updateRequest, {
                                                onError: (error) => {
                                                    enqueueSnackbar(`Unable to update service group: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, { variant: Severity.Error, action: SnackbarDismissButton })
                                                    reject()
                                                }
                                            })
                                            resolve()
                                ...
                                new Promise<void>((resolve, reject) => {
                                    setTimeout(() => {
                                        ...
                                        deleteServiceGroup.mutate(deleteRequest, {
                                            onError: (error) => {
                                                enqueueSnackbar(`Unable to delete service group: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, { variant: Severity.Error, action: SnackbarDismissButton })
                                                reject()
                                            }
                                        })
                                        resolve()
