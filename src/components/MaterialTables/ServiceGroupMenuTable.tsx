...
export default function ServiceGroupMaterialTable() {
    ...
    const [state, setState] = useState<{columns: any[], loader: boolean, data: serviceGroupColumns[]}>({
        columns,
        loader: true,
        data: []
    });
    ...
    function reloadSetter() {
        gRPCClients.serviceGroupClient.getAll(new GetAllRequest(), {}).then(serviceGroupResponse => {
                        ...
                        actions={[
                            {
                                ...
                                onClick: (event, rowData) => {
                                    return gRPCClients.serviceGroupClient.redeploy(new RedeployRequest().setId(new UUID().setValue((rowData as serviceGroupColumns).id as string)), {}).then(() => {
                            ...
                            onRowAdd: (newData) =>
                                new Promise<void>((resolve, reject) => {
                                    setTimeout(() => {
                                        ...
                                        gRPCClients.serviceGroupClient.store(storeRequest, {}).then(result => {
                                ...
                                new Promise<void>((resolve, reject) => {
                                    setTimeout(() => {
                                        if (oldData){
                                            ...
                                            gRPCClients.serviceGroupClient.update(updateRequest, {}).then(_ => {
                                                setState((prevState) => {
                                                    const data = [...prevState.data];
                            ...
                            onRowDelete: (oldData) =>
                                new Promise<void>((resolve, reject) => {
                                    setTimeout(() => {
                                        ...
                                        gRPCClients.serviceGroupClient.delete(deleteRequest, {}).then(_ => {
                                            setState((prevState) => {
                                                const data = [...prevState.data];
