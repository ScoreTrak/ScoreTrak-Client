...
import {IPropertyToProperty, propertyToIProperty} from "../../lib/material-table/properties";
...
export function PropertyMaterialTable() {
    ...
    const { data: propertiesData, isLoading: propertiesIsLoading, isSuccess: propertiesIsSuccess } = usePropertiesQuery()
    ...
    useEffect(() => {
        if (servicesData) {
            ...
            for (let i = 0; i < servicesData.length; i++) {
                ...
                lookup[service.getId()?.getValue() as string] = `${service.getName()} (ID: ${service.getId()?.getValue() as string}`
            }
                                ...
                                new Promise<void>((resolve, reject) => {
                                    setTimeout(() => {
                                        ...
                                        addProperty.mutate(storeRequest, {
                                            onError: (error) => {
                                                enqueueSnackbar(`Unable to store property: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, {
                                    ...
                                    setTimeout(() => {
                                        if (oldData) {
                                            ...
                                            updateProperty.mutate(updateRequest, {
                                                onError: (error) => {
                                                    enqueueSnackbar(`Unable to update property: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, {
                                                        variant: Severity.Error,
                                                        action: SnackbarDismissButton
                                                    })
                                                    reject()
                                ...
                                new Promise<void>((resolve, reject) => {
                                    setTimeout(() => {
                                        ...
                                        deleteProperty.mutate(deleteRequest, {
                                            onError: (error) => {
                                                enqueueSnackbar(`Unable to delete property: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, {
                                                    variant: Severity.Error,
                                                    action: SnackbarDismissButton
                                                })
                                                reject()
