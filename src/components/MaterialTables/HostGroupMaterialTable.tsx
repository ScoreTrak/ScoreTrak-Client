...
import {hostGroupToIHostGroup, IHostGroupToHostGroup} from "../../lib/material-table/host-groups";
                            ...
                            new Promise<void>((resolve, reject) => {
                                setTimeout(() => {
                                    ...
                                    addHostGroup.mutate(storeRequest, {
                                        onError: (error) => {
                                            enqueueSnackbar(`Unable to store hostGroup: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, { variant: Severity.Error, action: SnackbarDismissButton })
                                            reject()
                                        }
                                    })
                                    resolve()
                                ...
                                setTimeout(() => {
                                    if (oldData){
                                        ...
                                        updateHostGroup.mutate(updateRequest, {
                                            onError: (error) => {
                                                enqueueSnackbar(`Unable to update hostGroup: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, { variant: Severity.Error, action: SnackbarDismissButton })
                                                reject()
                                            }
                                        })
                                        resolve()
                            ...
                            new Promise<void>((resolve, reject) => {
                                setTimeout(() => {
                                    ...
                                    deleteHostGroup.mutate(deleteRequest, {
                                        onError: (error) => {
                                            enqueueSnackbar(`Unable to delete hostGroup: ${(error as grpcWeb.RpcError).message}. Error code: ${(error as grpcWeb.RpcError).code}`, { variant: Severity.Error, action: SnackbarDismissButton })
                                            reject()
                                        }
                                    })
                                    resolve()
