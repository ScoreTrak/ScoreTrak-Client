...
export type availableChecks =
  ...
  | "SQL"
  | "CalDav";
export type PropertyType = Record<
  string,
  {
    name: string;
    type: "field" | "select";
    defaultStatus?: IPropertyStatus;
    defaultValue?: string;
    options?: string[];
  }
>;
...
export const Checks: ChecksType = {
  WINRM: {
    ...
    Password: {
      name: "Password",
      type: "field",
      defaultStatus: IPropertyStatus.Edit,
    },
  ...
  SMB: {
    ...
    Operation: {
      name: "Operation",
      type: "select",
      defaultValue: "CreateAndOpen",
      options: ["Open", "Create", "CreateAndOpen"],
    },
    ExpectedOutput: { name: "Expected Output", type: "field" },
  },
  ...
  LDAP: {
    ...
    Filter: {
      name: "Filter",
      type: "field",
      defaultValue: "(&(objectClass=organizationalPerson))",
    },
